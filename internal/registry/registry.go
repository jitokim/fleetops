...
// weren't spawned via fleetops's "n" key have no record here, and render
...
// LoopsDir is ~/.fleetops/loops (override for tests by passing an
...
func LoopsDir() string {
	...
	return filepath.Join(home, ".fleetops", "loops")
}

// PendingDir is ~/.fleetops/pending (same override pattern as LoopsDir).
func PendingDir() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".fleetops", "pending")
}
...
type Record struct {
	...
	// false for every record created before this field existed, and false
	// by default even for a freshly-Bound record unless
	// the caller explicitly opts in (see BindSpec.Driven) — the
