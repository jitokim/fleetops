...
// PERSISTED record (written by any version of fleetops before this
...
// readable by anything (a human `cat`, an older fleetops binary) that
...
// TestSaveVerdict_PreservesDriven is the requirement: a driven
...
func TestWritePending_BindPending_DrivenSurvivesRoundTrip(t *testing.T) {
	...
	now := time.Now()
	if err := WritePending(pendingDir, "/x/myproject", BindSpec{Goal: "fix it", Driven: true}); err != nil {
...
func TestWritePending_ListPendingRoundTrip(t *testing.T) {
	dir := t.TempDir()
	if err := WritePending(dir, "/x/myproject", BindSpec{Goal: "fix the bug"}); err != nil {
		t.Fatalf("WritePending: %v", err)
	}
	...
	for _, p := range pending {
		if p.Cwd != "/x/myproject" || p.Goal != "fix the bug" {
			t.Errorf("got %+v, want {cwd:/x/myproject goal:fix the bug}", p)
		}
	}
}
...
func TestWritePending_ListPendingRoundTrip_FullContract(t *testing.T) {
	...
	if err := WritePending(dir, "/x/myproject", spec); err != nil {
		t.Fatalf("WritePending: %v", err)
	}
...
func TestBindPending_MatchesNewestUnboundSessionInCwd(t *testing.T) {
	...
	if err := WritePending(pendingDir, "/x/myproject", BindSpec{Goal: "fix the bug"}); err != nil {
		t.Fatalf("WritePending: %v", err)
	}
	...
	for name := range listPending(pendingDir) {
		if err := writePendingRaw(pendingDir, name, "/x/myproject", "fix the bug", spawnTS); err != nil {
			t.Fatalf("backdate pending: %v", err)
		}
	}

	loops := []domain.Loop{
		{SessionID: "older", Cwd: "/x/myproject", ProjectDir: "-x-myproject", LastActivity: spawnTS.Add(-time.Hour)}, // pre-existing, before the spawn — must NOT match
		{SessionID: "newer", Cwd: "/x/myproject", ProjectDir: "-x-myproject", LastActivity: spawnTS.Add(time.Second)},
...
func TestBindPending_SkipsAlreadyBoundSessions(t *testing.T) {
	...
	if err := WritePending(pendingDir, "/x/myproject", BindSpec{Goal: "new goal"}); err != nil {
		t.Fatalf("WritePending: %v", err)
	}
	for name := range listPending(pendingDir) {
		if err := writePendingRaw(pendingDir, name, "/x/myproject", "new goal", spawnTS); err != nil {
			t.Fatalf("backdate pending: %v", err)
		}
	}

	loops := []domain.Loop{
		{SessionID: "already-bound", Cwd: "/x/myproject", ProjectDir: "-x-myproject", LastActivity: spawnTS.Add(time.Second)},
	}
...
func TestBindPending_StalePendingDropped(t *testing.T) {
	...
	if err := WritePending(pendingDir, "/x/myproject", BindSpec{Goal: "goal"}); err != nil {
		t.Fatalf("WritePending: %v", err)
	}
	for name := range listPending(pendingDir) {
		if err := writePendingRaw(pendingDir, name, "/x/myproject", "goal", staleTS); err != nil {
			t.Fatalf("backdate pending: %v", err)
		}
	}

	// even a perfectly matching session must not be bound once stale.
	loops := []domain.Loop{{SessionID: "s1", Cwd: "/x/myproject", ProjectDir: "-x-myproject", LastActivity: staleTS.Add(time.Second)}}
	BindPending(loopsDir, pendingDir, loops, now, t.TempDir())
...
func TestBindPending_NoMatchYet_PendingSurvives(t *testing.T) {
	...
	for name := range listPending(pendingDir) {
		if err := writePendingRaw(pendingDir, name, "/x/myproject", "goal", spawnTS); err != nil {
			t.Fatalf("backdate pending: %v", err)
		}
	}
...
func TestBindPending_SuccessfulMatch_RecordsSpawnActuationEvent(t *testing.T) {
	...
	loops := []domain.Loop{{SessionID: "newer", Cwd: "/x/myproject", ProjectDir: "-x-myproject", State: domain.StateRunning, LastActivity: spawnTS.Add(time.Second)}}
...
func TestCapDetail_MultibyteText_NeverSplitsARune(t *testing.T) {
	// a goal made of multi-byte (CJK) runes, longer than detailCap runes —
	// a byte-index cut would slice a multi-byte character in half and
	// corrupt the tail.
	long := strings.Repeat("字", detailCap+10)
	got := capDetail(long)
