...
func TestCmuxController_Spawn_ReturnsUnsupportedError(t *testing.T) {
	err := (cmuxController{}).Spawn("/x/myproject", "do the thing")
	if err == nil {
...
func TestLocateCmux_TerminalSurfaceMatchingCwd_Found(t *testing.T) {
	...
	got, ok := locateCmux(realCmuxTree(t), "-home-user-myproject", resolve)
	if !ok {
		t.Fatal("expected ok=true")
	}
	want := Target{Backend: "cmux", ID: "surface:15", Cwd: "/home/user/myproject", Window: "window:1"}
	if got != want {
		t.Errorf("got %+v, want %+v", got, want)
	}
}
...
func TestLocateCmux_NoTTYResolves_NotFound(t *testing.T) {
	...
	got, ok := locateCmux(realCmuxTree(t), "-home-user-myproject", stubResolver(nil))
	if ok {
...
func TestLocateCmux_BrowserSurfaceNeverReturned(t *testing.T) {
	...
	resolve := stubResolver(map[string]ttyResolution{
		"ttys009": {cwd: "/home/user/myproject", hasClaude: false},
	})
	got, ok := locateCmux(realCmuxTree(t), "-home-user-myproject", resolve)
	if !ok {
...
func TestLocateCmuxClaude_SingleClaudeSurface_Found(t *testing.T) {
	...
	got, ok := locateCmuxClaude(realCmuxTree(t), "-home-user-myproject", resolve)
	if !ok {
...
func TestLocateCmuxClaude_NoClaudeAttached_NotFound_ButLocateFinds(t *testing.T) {
	...
	if _, ok := locateCmuxClaude(realCmuxTree(t), "-home-user-myproject", resolve); ok {
		t.Error("expected ok=false — no claude process attached to the surface")
	}
	if _, ok := locateCmux(realCmuxTree(t), "-home-user-myproject", resolve); !ok {
...
func TestLocateCmuxClaude_SameTTYTwoSurfaces_NotAmbiguous(t *testing.T) {
	...
	got, ok := locateCmuxClaude(realCmuxTree(t), "-home-user-team", resolve)
	if !ok {
	...
	if got.Cwd != "/home/user/team" {
		t.Errorf("got Cwd %q, want /home/user/team", got.Cwd)
	}
...
func TestLocateCmuxClaude_DotContainingPath_Matches(t *testing.T) {
	...
	resolve := stubResolver(map[string]ttyResolution{
		"ttys008": {cwd: "/home/user/.someplugin/agent-sessions", hasClaude: true},
	})
	got, ok := locateCmuxClaude(realCmuxTree(t), "-home-user--someplugin-agent-sessions", resolve)
	if !ok {
...
func TestParseCmuxPsRows_FiltersTTYsAndDetectsClaudeAndForeground(t *testing.T) {
	...
	out := "" +
		...
		"ttys001  S+    1626 /home/user/.local/bin/claude\n" +
		"ttys001  S+    1982 npm exec figma-developer-mcp --stdio\n" +
		"ttys002  S+    9001 /home/user/.local/bin/claude\n" +
		"??       Ss    4242 /some/daemon\n"
...
func TestParseLsofPidCwds(t *testing.T) {
	out := "p1626\nfcwd\nn/home/user/otherproject\np15961\nfcwd\nn/home/user/.someplugin\n"
	got := parseLsofPidCwds(out)
	want := map[int]string{
		1626:  "/home/user/otherproject",
		15961: "/home/user/.someplugin",
	}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("got %+v, want %+v", got, want)
	}
