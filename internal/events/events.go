...
// under ~/.fleetops/history/<session_id>.jsonl — same one-file-per-entity,
...
// HistoryDir is ~/.fleetops/history (override for tests by passing an
...
func HistoryDir() string {
	...
	return filepath.Join(home, ".fleetops", "history")
}
...
const (
	...
	// timeline and `fleetops report` can tell "a human drove this" from
	// "the engine drove this" apart at a glance, without inspecting Detail
	// text. Emitted by the engine's drive command each time it fires a
	// cycle.
...
// fleetops run is already far outside normal usage; keeping one backup
...
// within one fleetops process (the scanner's transition detector, every
...
// and sorted oldest-first per session. Used by `fleetops report`. Tolerant
...
// paraphrased); cmd/fleetops's report command only ever needed the
