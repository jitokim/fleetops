...
> implemented.** fleetops today is 100% Go. It has a minimal, opt-in
> LoopEngine (see the README's "Engine" section) — a governance harness that
> drives a loop's cycles on progress verdicts, nothing more — but no daemon
> and no persistence store beyond flat files. The Python/Textual/asyncio
> stack, richer `LoopEngine`, and `Store` below are a design sketch for where
> the project could go, not a spec for what exists. For the actual current
> behavior, see [`README.md`](./README.md) and [`DESIGN.md`](./DESIGN.md).
...
- stack: Python 3.11+ · Textual (TUI) · asyncio (engine) · LLM calls via a
  pluggable gateway (any OpenAI-compatible or provider-native gateway)
...
matter. fleetops packages exactly that: **oracle + challenger + governance +
...
Some directions beyond what's built today (see README/DESIGN.md for the
current, much smaller engine):

- A real `Governor` + `Agent` + `Oracle` running as a headless engine, with
  the TUI as a thin client that attaches to observe and answer gates rather
  than owning the loop itself.
- The engine outliving the TUI process — a daemon, so an overnight fleet of
  loops keeps running with nobody watching, and gate notifications can go
  wherever you actually are (Slack, a phone push) instead of only the
  terminal.
- More agent adapters beyond a single CLI (headless variants of other coding
  agents), a real `Challenger` pass, session replay/drill-down, loop config
  files, and richer governance (e.g. circuit-breaking a flapping loop).
