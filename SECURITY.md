...
## What makes missionctl security-relevant

missionctl is not a passive dashboard. Its actuation layer
...
- **Injection via untrusted input**: anything that lets content from a
  ...
  command missionctl runs or what gets sent to a terminal, instead of being
...
- **Hook installation risk**: `missionctl hooks install` edits
...
Preferred: open a [GitHub private security advisory](https://github.com/jitokim/missionctl/security/advisories/new)
...
subject line starting `[missionctl security]`.

Please include:
- What backend/action was involved (orca / tmux / cmux / bare terminal;
  attach / resume / approve / stop / kill / spawn).
- Steps to reproduce, including relevant versions (missionctl commit, OS,
...
- Impact as you understand it (e.g. "sent keystrokes to an unintended
  terminal" vs. "crashed missionctl").
...
In scope: missionctl's own code (`cmd/`, `internal/`) — actuation logic,
