# Contributing to missionctl

missionctl is a small, 0.1.0-alpha Go CLI. Contributions — bug reports,
...
```bash
git clone https://github.com/jitokim/missionctl.git
cd missionctl
go build -o missionctl ./cmd/missionctl
./missionctl
```

Requires Go 1.25+ (see `go.mod`). No other toolchain or codegen step.
...
- missionctl version / commit, OS, and which backend (orca / tmux / cmux /
