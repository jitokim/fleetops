...
const (
	testCmd     = "/usr/local/bin/missionctl hook notify"
	testStartC  = "/usr/local/bin/missionctl hook session-start"
	testEndCmd  = "/usr/local/bin/missionctl hook session-end"
...
// case verified live against the captain's settings: another tool already
...
// idempotence the live re-install against the captain's settings relies on.
...
// ── isMissionctlHookCommand ───────────────────────────────────────────────

func TestIsMissionctlHookCommand(t *testing.T) {
	cases := []struct {
		cmd    string
		suffix string
		want   bool
	}{
		{"/usr/local/bin/missionctl hook notify", notifySuf, true},
		{"missionctl hook notify", notifySuf, true},
		{"some-other-tool notify", notifySuf, false},
		{"missionctl hook resume", notifySuf, false}, // doesn't end in "hook notify"
		{"/opt/other/missionctl-lookalike hook notify", notifySuf, true},
		{"/usr/local/bin/missionctl hook session-start", startSuffix, true},
		{"/usr/local/bin/missionctl hook session-end", endSuffix, true},
		// wrong suffix for the command: notify must NOT match session-start's
		// suffix, so the events never cross-remove each other.
		{"/usr/local/bin/missionctl hook notify", startSuffix, false},
		{"/usr/local/bin/missionctl hook session-start", notifySuf, false},
	}
	for _, c := range cases {
		if got := isMissionctlHookCommand(c.cmd, c.suffix); got != c.want {
			t.Errorf("isMissionctlHookCommand(%q, %q) = %v, want %v", c.cmd, c.suffix, got, c.want)
		}
	}
}

// TestMissionctlHookSpecs pins the exact set of managed events + suffixes so a
// future edit that drops or renames one is caught here.
func TestMissionctlHookSpecs(t *testing.T) {
	specs := missionctlHookSpecs()
