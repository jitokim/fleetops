...
// runReportCmd implements `missionctl report --since 24h`: an OFFLINE,
...
func runReportCmd(args []string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "missionctl report: invalid --since %q: %v\n", *since, err)
		os.Exit(1)
	}
	if err := writeReport(os.Stdout, events.HistoryDir(), *since, window, time.Now()); err != nil {
		fmt.Fprintln(os.Stderr, "missionctl report:", err)
		os.Exit(1)
	}
}
...
func writeReport(w io.Writer, dir, sinceLabel string, window time.Duration, now time.Time) error {
	...
	fmt.Fprintf(w, "missionctl report — since %s (%d loop%s)\n", sinceLabel, len(summaries), plural(len(summaries)))
